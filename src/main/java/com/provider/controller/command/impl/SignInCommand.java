...
public class SignInCommand extends FrontCommand {
    ...
    public @NotNull CommandResult execute() throws ServletException, IOException, DBException, CommandParamException {
        final Map<String, String> paramMap = getRequiredParams(UserParams.LOGIN, UserParams.PASSWORD);
        final String login = paramMap.get(UserParams.LOGIN);
        final String password = paramMap.get(UserParams.PASSWORD);
